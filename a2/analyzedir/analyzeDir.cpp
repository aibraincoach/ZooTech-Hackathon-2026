...
Results analyzeDir(int n)
{
  ...
  res.largest_dirs.push_back({"path1", 150});

  res.most_common_words.push_back({ "world", 1 });
  res.most_common_words.push_back({ "hello", 3 });
  //Comparator the sorts on size count first (second of pair, then on name by str::string natural order)
  struct
  {
    bool operator()(std::pair<std::string, int> a, std::pair<std::string, int> b) const
    {
      //Need to check if equal first
      bool comp = a.second == b.second;
      if (!comp)
      {
        //If not equal then return larger in front of smaller as correct
        return a.second > b.second;
      }
      //If equal then do secondary name ordering
      return a.first.compare(b.first) < 0 ? true : false;
    }
  } customPairSortWords;
  std::sort(res.most_common_words.begin(), res.most_common_words.end(), customPairSortWords);
